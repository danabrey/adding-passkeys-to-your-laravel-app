<section>
    ...
    <form x-data="registerPasskey" x-show="browserSupportsWebAuthn()" x-on:submit.prevent="register($el)" name="createPasskey" method="post" action="{{ route('passkeys.store') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="create_passkey_passkey_name" name="name" x-model="name" class="mt-1 block w-full"/>
    ...
    <div class="mt-6">
        ...
        <ul class="mt-2">
            ...
            <li class="px-2 py-2 flex justify-between items-center">
                ...
                <form method="post" action="{{ route('passkeys.destroy', $passkey) }}" class="flex items-center">
                    @csrf
                    @method('DELETE')
